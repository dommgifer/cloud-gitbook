...
### 混沌工程
混沌工程簡單的說「故意在系統裡製造可控的壞事」，來驗證你的分散式系統和 Kubernetes 在出包時撐不撐得住、監控的告警不告得出來
大致有以下幾個概念
...
```
4. 編輯values.yaml，根據 K8S 使用的container runtime 做設定
此範例使用的是 docker
```
runtime: docker
...
![dashboard-token-guide](../.gitbook/assets/chaos-mesh/dashboard-token-guide.png)

8. 根據所需權限，選擇namespace或是整個 Cluster，並按照步驟建立rbac 和 token
![rbac-token-setup](../.gitbook/assets/chaos-mesh/rbac-token-setup.png)

9. 取得token後輸入，即可登入
![token-login](../.gitbook/assets/chaos-mesh/token-login.png)
...
![web-show-demo](../.gitbook/assets/chaos-mesh/web-show-demo.png)


## 建立網路延遲實驗
1. 建立一個實驗，類型為網路攻擊的延遲，設為 10ms
![network-delay-config](../.gitbook/assets/chaos-mesh/network-delay-config.png)
2. 目標設定為 web-show pod，持續 10s，然後建立
![network-delay-target](../.gitbook/assets/chaos-mesh/network-delay-target.png)
3. 查看web-show 發現 ping 的封包確實有持續10s延遲
![network-delay-result](../.gitbook/assets/chaos-mesh/network-delay-result.png)
4. 回到實驗頁面，看到實驗已執行完成
![network-delay-complete](../.gitbook/assets/chaos-mesh/network-delay-complete.png)

## 建立網路封包遺失實驗
1. 建立一個實驗，類型為網路封包遺失，遺失率為 100%
![network-loss-config](../.gitbook/assets/chaos-mesh/network-loss-config.png)
2. 目標設定為 web-show pod，持續 10s，然後建立
![network-loss-target](../.gitbook/assets/chaos-mesh/network-loss-target.png)
3. 查看 web-show，確定封包有遺失
![network-loss-result](../.gitbook/assets/chaos-mesh/network-loss-result.png)



## 建立 Pod Failure 實驗
1. 建立 Pod Fault 類型的 pod Failure 實驗
![pod-failure-config](../.gitbook/assets/chaos-mesh/pod-failure-config.png)
2. 選定 web-show pod，持續時間為永久，送出實驗
![pod-failure-target](../.gitbook/assets/chaos-mesh/pod-failure-target.png)
3. 查看 pod 狀態
![pod-failure-status](../.gitbook/assets/chaos-mesh/pod-failure-status.png)
4. 查看實驗狀態
![pod-failure-experiment](../.gitbook/assets/chaos-mesh/pod-failure-experiment.png)
5. 停止實驗，確認 pod 恢復正常
![pod-failure-recovered](../.gitbook/assets/chaos-mesh/pod-failure-recovered.png)

## 建立 Pod CPU Memory 壓力測試實驗
1. 建立 Stress Test 實驗，讓 Pod CPU 和 Memory 滿載
![stress-test-config](../.gitbook/assets/chaos-mesh/stress-test-config.png)
2. 選定 web-show pod，持續時間為永久，送出實驗
![stress-test-target](../.gitbook/assets/chaos-mesh/stress-test-target.png)
3. 此範例程式的資源 limit 為 cpu 0.3 core memort 20Mi![stress-test-limit](../.gitbook/assets/chaos-mesh/stress-test-limit.png)
4. 查看pod監控值，CPU Memory都滿載
![stress-test-result](../.gitbook/assets/chaos-mesh/stress-test-result.png)
...
![workflow-ui](../.gitbook/assets/chaos-mesh/workflow-ui.png)
不過以下範例是透過 yaml 來制定
...
![workflow-result](../.gitbook/assets/chaos-mesh/workflow-result.png)
4. 查看 workflow 頁面的實驗拓墣圖
![workflow-topology](../.gitbook/assets/chaos-mesh/workflow-topology.png)
...
安裝 Chaos DNS Service 後，才會看到 DNS的錯誤實驗
可以選擇 error

先測試解析錯誤，測試解析到 google.com 有錯誤
![dns-error-config](../.gitbook/assets/chaos-mesh/dns-error-config.png)
指定服務
![dns-error-target](../.gitbook/assets/chaos-mesh/dns-error-target.png)

這是在注入 DNS 錯誤前，從pod 內部 ping google.com ，可以解析
![dns-before-inject](../.gitbook/assets/chaos-mesh/dns-before-inject.png)

注入錯誤後，就無法解析了
![dns-after-inject](../.gitbook/assets/chaos-mesh/dns-after-inject.png)


測試 DNS Random 返回 IP，這次輸入*，所有的 domain name 會隨機返回 IP
![dns-random-config](../.gitbook/assets/chaos-mesh/dns-random-config.png)
指定服務
![dns-random-target](../.gitbook/assets/chaos-mesh/dns-random-target.png)
...
易於使用：您只需要在 Chaosd 中執行簡單的命令即可創建和管理混沌實驗。
各種故障類型：Chaosd 提供各種故障類型，可以注入到不同級別的物理機器中，包括進程、網絡、壓力、磁盤、主機等。將添加更多的故障類型。
多種工作模式：Chaosd 可以作為命令行工具和服務使用，以滿足不同場景的需求。
支持的故障類型
您可以使用 Chaosd 模擬以下故障類型：

進程：將故障注入到進程中。支持操作如終止進程或停止進程等。
網絡：將故障注入到物理機器的網絡中。支持操作如增加網絡延遲、丟包和損壞包等。
壓力：對物理機器的 CPU 或內存施加壓力。
磁盤：將故障注入到物理機器的磁盤中。支持操作如增加讀寫磁盤負載和填充磁盤等。
主機：對物理機器施加故障。支持操作如關閉物理機器等。
...
2. 透過 chaosctl 產生 TLS 憑證，並註冊至 chaos-mesh
   執行 Chaosctl 的機器可以存取Kubernetes 叢集，並且可以使用SSH 工具連接到實體機。
   以下操作會產生Chaosd 所需的證書，並把證書存到對應的實體機上，並在Kubernetes 叢集中建立對應的PhysicalMachine資源。
...
![chaosd-cpu-stress-config](../.gitbook/assets/chaos-mesh/chaosd-cpu-stress-config.png)

2. 選擇目標機器，設定持續時間，送出

![chaosd-cpu-stress-target](../.gitbook/assets/chaos-mesh/chaosd-cpu-stress-target.png)

3. 到目標機器，可以看到 chaosd 被呼叫了建立實驗 API
    ![chaosd-cpu-stress-api](../.gitbook/assets/chaos-mesh/chaosd-cpu-stress-api.png)
   查看機器的 cpu，確實有被加壓到80%
    ![chaosd-cpu-stress-result](../.gitbook/assets/chaos-mesh/chaosd-cpu-stress-result.png)

#### Network dekay
1. 選擇 Hosts -> Network Attack -> delay
![chaosd-network-delay-menu](../.gitbook/assets/chaos-mesh/chaosd-network-delay-menu.png)

2. 設定延遲，指定網卡，這裡設定往 192.168.1.20(master)和192.168.1.21(worker1)的網路延遲10000ms
![chaosd-network-delay-config](../.gitbook/assets/chaos-mesh/chaosd-network-delay-config.png)

3.選擇目標機器，設定持續時間，送出
![chaosd-network-delay-target](../.gitbook/assets/chaos-mesh/chaosd-network-delay-target.png)

4. 以上的結果會是 讓 worker4 送往(或收到) master 和 worker1 的網路延遲 10000ms

5. 查看 k8s 的 節點狀態，可以看到 worker4有問題
![chaosd-network-delay-result](../.gitbook/assets/chaos-mesh/chaosd-network-delay-result.png)
...
1. 選擇 Hosts -> Network Attack
    ![chaosd-network-loss-menu](../.gitbook/assets/chaos-mesh/chaosd-network-loss-menu.png)
2. 設定封包遺失率，指定網卡，這裡設定從 192.168.1.19 來的封包(或送出)，遺失率50%，其餘設定請看官網
![chaosd-network-loss-config](../.gitbook/assets/chaos-mesh/chaosd-network-loss-config.png)

3. 選擇目標機器，設定持續時間，送出
![chaosd-network-loss-target](../.gitbook/assets/chaos-mesh/chaosd-network-loss-target.png)
...
![chaosd-network-loss-status](../.gitbook/assets/chaos-mesh/chaosd-network-loss-status.png)
